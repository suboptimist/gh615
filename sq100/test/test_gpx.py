...
def make_gpx_root(
    ...
    return gpx.GpxRoot(metadata=metadata or make_metadata(), tracks=tracks or [])
...
def test_create_datetime_element_without_tzinfo() -> None:
    dt = datetime.datetime(year=1987, month=12, day=19, hour=15, minute=30, second=20)
    elem = gpx.make_datetime_element(ns="timo", tag="birth", value=dt)
    assert elem.tag == "{timo}birth"
...
def test_gpx_root__to_etree() -> None:
    gpx_root = gpx.GpxRoot(
        ...
        tracks=[
            make_track(
                number=11, track_points=[make_track_point(latitude=-1, longitude=2)]
            ),
            make_track(
                number=22, track_points=[make_track_point(latitude=3, longitude=-4)]
            ),
        ],
    )
...
def test_metadata__to_etree() -> None:
    metadata = gpx.Metadata(
        ...
        bounds=gpx.Bounds(minlat=1.0, minlon=2.0, maxlat=3.0, maxlon=4.0),
    )
...
def test_make_track_segment_element() -> None:
    track_points = [
        make_track_point(latitude=1, time=datetime.datetime(2000, 1, 1, 12, 1, 1)),
        make_track_point(latitude=2, time=datetime.datetime(2000, 1, 1, 12, 2, 2)),
    ]
...
def test_calc_track_bounds__returns_correct_bounds() -> None:
    bounds = gpx.calc_tracks_bounds(
        tracks=[
            make_track(track_points=[make_track_point(latitude=1.0, longitude=2.0)]),
            make_track(track_points=[make_track_point(latitude=3.0, longitude=-4.0)]),
        ]
    )
