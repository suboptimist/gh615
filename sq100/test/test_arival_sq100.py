...
def make_message(command: int = 0x00, parameter: bytes = b"") -> arival_sq100.Message:
    return arival_sq100.Message(
        command=command, payload_length=0, parameter=parameter, checksum=0
    )
...
def test_is_get_tracks_finish_message__returns_false_for_other_command() -> None:
...
def test_process_get_tracks_lap_info_msg__returns_correct_laps(
    ...
    unpack_lap_info_mock = mocker.patch("sq100.arival_sq100.unpack_lap_info_parameter")
...
def test_process_get_tracks_lap_info_msg__raises_if_tracks_are_incompatible(
    ...
    with pytest.raises(SQ100MessageException):
        arival_sq100.process_get_tracks_lap_info_msg(
            expected_track_info=make_track_info(no_laps=3), msg=msg
        )
...
def test_process_get_tracks_track_points_msg__returns_points(
    ...
    unpack_mock = mocker.patch("sq100.arival_sq100.unpack_track_point_parameter")
    returned_track = make_track_info()
    returned_track_points = make_track_points(
        [{"latitude": 3.0}, {"latitude": 4.0}, {"latitude": 5.0}]
    )
    ...
    assert track_points[0].latitude == 3.0
    assert track_points[1].latitude == 4.0
    assert track_points[2].latitude == 5.0
...
def test_process_get_tracks_track_points_msg__raises_if_session_len_is_wrong(
    ...
    with pytest.raises(SQ100MessageException):
        arival_sq100.process_get_tracks_track_points_msg(
            expected_track_info=track, expected_session_start=0, msg=msg
        )
...
def make_track_point_pack(
    ...
    return struct.pack(
        ...
        heart_rate,
        0,
        int(round(interval.total_seconds() * 10)),
        b"",
    )
...
def test_unpack_track_point_parameter__unpacks_correct_data() -> None:
    parameter = (
        make_track_point_trackhead_pack(
            ...
            no_track_points=1,
            duration=datetime.timedelta(seconds=100.0),
            distance=2,
            no_laps=3,
            session_start=4,
            session_last=5,
        ...
        + make_track_point_pack(
            ...
            speed=11.11,
            heart_rate=102,
            interval=datetime.timedelta(seconds=10.1),
        )
        + make_track_point_pack(
            latitude=-12.345678,
            longitude=-87.654321,
    ...
    track, session, track_points = arival_sq100.unpack_track_point_parameter(parameter)
...
def test_unpack_track_list_parameter__unpacks_correct_data() -> None:
    parameter = make_track_list_entry_pack(
...
def make_message_pack(
    command: int, parameter: bytes, payload_length: int, checksum: int
) -> bytes:
    return struct.pack(
        ">BH%dsB" % len(parameter), command, payload_length, parameter, checksum
    )
...
def test_query__calls_correct_write(mocker: MockerFixture) -> None:
    mock_connection = mocker.create_autospec(serial_connection.SerialConnection)
    mock_connection.read.side_effect = [
        bytes([0, 0, 2]),
        b"hi" + bytes([2 ^ ord("h") ^ ord("i")]),
    ]

    arival_sq100.query(connection=mock_connection, command=42, parameter=b"ab")
...
def test_query__returns_correct_message(mocker: MockerFixture) -> None:
    ...
    message = arival_sq100.query(
        connection=mock_connection, command=42, parameter=b"ab"
    )
