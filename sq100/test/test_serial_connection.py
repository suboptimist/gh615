...
def test_query__returns_data_if_all_succesfull(mocker: MockerFixture) -> None:
    mock_serial = mocker.patch("serial.Serial")
    serial_instance = mock_serial.return_value
    serial_instance.read.return_value = b'\xb0\xb1'
    config = SerialConfig(port="port", baudrate=10, timeout=11.11)
    connection = SerialConnection(config)
    assert connection.query(b'\xa0\xa1') == b'\xb0\xb1'
    serial_instance.write.assert_called_once_with(b'\xa0\xa1')
    serial_instance.read.assert_called_once()


def test_query__returns_data_if_succesfull_after_three_trials(
        mocker: MockerFixture
) -> None:
    mock_serial = mocker.patch("serial.Serial")
    serial_instance = mock_serial.return_value
    serial_instance.read.side_effect = ["", "", b'\xb0\xb1']
    config = SerialConfig(port="port", baudrate=10, timeout=11.11)
    connection = SerialConnection(config)
    assert connection.query(b'\xa0\xa1') == b'\xb0\xb1'
    serial_instance.write.assert_called_with(b'\xa0\xa1')
    assert serial_instance.write.call_count == 3
    assert serial_instance.read.call_count == 3


def test_query__raises_if_three_times_no_data_received(
        mocker: MockerFixture
) -> None:
    mock_serial = mocker.patch("serial.Serial")
    serial_instance = mock_serial.return_value
